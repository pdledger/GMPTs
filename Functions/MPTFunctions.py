...
def Theta1_SweepR3(Array,mesh,fes,fes2,Theta0Sols,xivec,alpha,sigma,mu,inout,Tolerance,Maxsteps,epsi,Solver,N0,TotalNOF,Vectors,Tensors,Multi,BP):
    ...
    if Tensors == True:
        R=np.zeros([3,3])
        I=np.zeros([3,3])
        TensorArrayR2=np.zeros([NOF,9], dtype=complex)
    ...
    Theta0i = GridFunction(fes)
    Theta0j = GridFunction(fes)
    Theta0k = GridFunction(fes)
    Theta1i = GridFunction(fes2)
    Theta1j = GridFunction(fes2)
    Theta1k = GridFunction(fes2)
    Theta1 = GridFunction(fes2)
    Theta2 = GridFunction(fes2)
    Theta3 = GridFunction(fes2)
    ...
    for k,Omega in enumerate(Array):
        ...
        if Tensors == True:
            ...
            #Save in arrays
            TensorArrayR2[k,:] = (N0+R+1j*I).flatten()
            EigenValues[k,:] = np.sort(np.linalg.eigvals(N0+R))+1j*np.sort(np.linalg.eigvals(I))
    ...
    if Tensors == True and Vectors == True:
        return TensorArrayR2, TensorArrayR3, TensorArrayR4, EigenValues, Theta1Sols
    elif Tensors == True:
        return TensorArrayR2, TensorArrayR3, TensorArrayR4, EigenValues
    else:
        return Theta1Sols
...
#            print(CR3)
    ...
    if PODErrorBars == True:
        return TensorArrayR2, TensorArrayR3, TensorArrayR4, EigenValues, ErrorTensors
    else:
        return TensorArrayR2, TensorArrayR3, TensorArrayR4, EigenValues
...
#Outputs -R as a numpy array
#        -I as a numpy array (This contains real values not imaginary ones)
def MPTCalculatorR3(mesh,fes,fes2,Theta1E1Sol,Theta1E2Sol,Theta1E3Sol,Theta0Sol,xivec,alpha,mu,sigma,inout,nu,tennumber,outof):
    #Print the progress of the sweep
    try:#This is used for the simulations run in parallel
        tennumber.value+=1
        print(' calculating tensor %d/%d    ' % (tennumber.value,outof), end='\r')
        ...
        for j in range(3):
            for k in range(3):
                ...
                if k==2:
                    Theta1k.vec.FV().NumPy()[:]=Theta1E3Sol
                #CR3[i,j,k] = 1j*(alpha**3)/4*Integrate(nu*sigma*inout*((Theta1k+Theta0k+xik)*xii),mesh)
                CR3[i,j,k] = 1j*(alpha**4)/8*Integrate(nu*sigma*inout*(xyz*ej)*((Theta1k+Theta0k+xik)*xii),mesh)
                NR3[i,j,k]=-alpha**4*Integrate((1-mu**(-1))*InnerProduct(xyz,ej)*InnerProduct(evec[i],1/2*curl(Theta0k)+1/2*curl(Theta1k)+evec[k])*inout,mesh)
