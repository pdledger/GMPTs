...
def PODEigPlotter(savename,Array,PODArray,EigenValues,PODEigenValues,EddyCurrentTest):
    ...
    Title, Show, ETP, _, MLS, MMS, SLS, SMS, _, _, ECL, IR3,JR3,KR3,IR4,JR4,KR4,LR4 = PlotterSettings()
...
def EigPlotter(savename,Array,EigenValues,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, ETP, _, MLS, MMS, _, _, _, _, ECL, IR3,JR3,KR3,IR4,JR4,KR4,LR4 = PlotterSettings()
...
def PODTensorPlotterR2(savename,Array,PODArray,Values,PODValues,EddyCurrentTest):
    #Create a way to reference xkcd colours
    PYCOL=['#1f77b4','#ff7f0e','#2ca02c','#d62728','#9467bd','#8c564b','#e377c2','#7f7f7f','#bcbd22','#17becf']

    #Retrieve the settings for the plot
    #Title, Show, _, TTP,MLS, MMS, SLS, SMS, _, _, ECL = PlotterSettings()
    Title, Show, _, TTP, MLS, MMS, SLS, SMS, _, _, ECL, IR3,JR3,KR3,IR4,JR4,KR4,LR4 = PlotterSettings()
...
def PODTensorPlotter(savename,Array,PODArray,ValuesR3,ValuesR4,PODValuesR3,PODValuesR4,EddyCurrentTest):
    #Create a way to reference xkcd colours
    PYCOL=['#1f77b4','#ff7f0e','#2ca02c','#d62728','#9467bd','#8c564b','#e377c2','#7f7f7f','#bcbd22','#17becf']
    ...
    Points=len(Array)
    NvalR3=len(IR3)
    DataR3=np.zeros([Points,NvalR3],dtype=complex)
    PODPoints=len(PODArray)
    PODDataR3=np.zeros([PODPoints,NvalR3],dtype=complex)
    for nval in range(NvalR3):
        i=IR3[nval]
        j=JR3[nval]
        k=KR3[nval]
        # Access appropriate value of tensor coefficent (i,j,k)
        index=9*(i-1)+3*(j-1)+k
        for nf in range(Points):
            DataR3[nf,nval] = ValuesR3[nf,index-1]
        for nf in range(PODPoints):
            PODDataR3[nf,nval] = PODValuesR3[nf,index-1]

    NvalR4=len(IR4)
    DataR4=np.zeros([Points,NvalR4],dtype=complex)
    PODDataR4=np.zeros([PODPoints,NvalR4],dtype=complex)
    for nval in range(NvalR4):
        i=IR4[nval]
        j=JR4[nval]
        k=KR4[nval]
        l=LR4[nval]
        # Access appropriate value of tensor coefficent (i,j,k)
        index=27*(i-1)+9*(j-1)+3*(k-1)+l
        for nf in range(Points):
            DataR4[nf,nval] = ValuesR4[nf,index-1]
            for nf in range(PODPoints):
                PODDataR4[nf,nval] = PODValuesR4[nf,index-1]



    #Plot the graph rank 4
    fig, ax = plt.subplots()

    #Plot the mainlines
    for i in range(NvalR3):
        if i==0:
            lines = ax.plot(Array,DataR3[:,i].real,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,DataR3[:,i].real,MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i in range(NvalR3):
        lines += ax.plot(PODArray,PODDataR3[:,i].real,SLS,markersize=SMS,color=PYCOL[i])
    ...
    if Title==True:
        plt.title(r"Tensor coefficients of Re($\mathcal{C}$) rank 3")

    #Create the legend
    names = []
    for i in range(NvalR3):
        names.append(r"Re($\mathcal{C}_{"+str(IR3[i])+","+str(JR3[i])+","+str(KR3[i])+"}(\omega)$) POD")
    for i in range(NvalR3):
        names.append(r"Re($\mathcal{C}_{"+str(IR3[i])+","+str(JR3[i])+","+str(KR3[i])+"}(\omega)$) Snapshot")
    ...
    #Save the graph
    plt.savefig(savename+"RealTensorCoeficientsR3.pdf")

    #Plot the imaginary graph rank 3
    fig, ax = plt.subplots()

    #Plot the mainlines
    for i in range(NvalR3):
        if i==0:
            lines = ax.plot(Array,DataR3[:,i].imag,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,DataR3[:,i].imag,MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i in range(NvalR3):
        lines += ax.plot(PODArray,PODDataR3[:,i].imag,SLS,markersize=SMS,color=PYCOL[i])
    ...
    if Title==True:
        plt.title(r"Tensor coefficients of Im($\mathcal{C}$) rank 3")

    #Create the legend
    names = []
    for i in range(NvalR3):
        names.append(r"Im($\mathcal{C}_{"+str(IR3[i])+","+str(JR3[i])+","+str(KR3[i])+"}(\omega)$) POD")
    for i in range(NvalR3):
        names.append(r"Im($\mathcal{C}_{"+str(IR3[i])+","+str(JR3[i])+","+str(KR3[i])+"}(\omega)$) Snapshot")
    ...
    #Save the graph
    plt.savefig(savename+"ImaginaryTensorCoeficientsR3.pdf")


    #Plot the graph rank 3
    fig, ax = plt.subplots()

    #Plot the mainlines
    for i in range(NvalR4):
        if i==0:
            lines = ax.plot(Array,DataR4[:,i].real,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,DataR4[:,i].real,MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i in range(NvalR4):
        lines += ax.plot(PODArray,PODDataR4[:,i].real,SLS,markersize=SMS,color=PYCOL[i])
    ...
    names = []
    for i in range(NvalR4):
        names.append(r"Re($\mathcal{C}_{"+str(IR4[i])+","+str(JR4[i])+","+str(KR4[i])+","+str(LR4[i])+"}(\omega)$) POD")
    for i in range(NvalR4):
        names.append(r"Re($\mathcal{C}_{"+str(IR4[i])+","+str(JR4[i])+","+str(KR4[i])+","+str(LR4[i])+"}(\omega)$) Snapshot")
    ...
    #Save the graph
    plt.savefig(savename+"RealTensorCoeficientsR4.pdf")
    ...
    for i in range(NvalR4):
        if i==0:
            lines = ax.plot(Array,DataR4[:,i].imag,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,DataR4[:,i].imag,MLS,markersize=MMS,color=PYCOL[i])

    #Plot the snapshots
    for i in range(NvalR4):
        lines += ax.plot(PODArray,PODDataR4[:,i].imag,SLS,markersize=SMS,color=PYCOL[i])
    ...
    names = []
    for i in range(NvalR4):
        names.append(r"Im($\mathcal{C}_{"+str(IR4[i])+","+str(JR4[i])+","+str(KR4[i])+","+str(LR4[i])+"}(\omega)$) POD")
    for i in range(NvalR4):
        names.append(r"Im($\mathcal{C}_{"+str(IR4[i])+","+str(JR4[i])+","+str(KR4[i])+","+str(LR4[i])+"}(\omega)$) Snapshot")
    ...
    #Save the graph
    plt.savefig(savename+"ImaginaryTensorCoeficientsR4.pdf")
...
def TensorPlotter(savename,Array,Values,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, _, TTP, MLS, MMS, _, _, _, _, ECL, IR3,JR3,KR3,IR4,JR4,KR4,LR4 = PlotterSettings()

    #Extract the data to be plotted
    Points=len(Array)
    Nval=len(I)
    Data=np.zeros([Points,Nval],dtype=complex)
    for nval in range(Nval):
        ...
        index=9*(i-1)+3*(j-1)+k
        for nf in range(Points):
            Data[nf,nval] = Values[nf,index-1]
    ...
    for i in range(Nval):
        if i==0:
            lines = ax.plot(Array,Data[:,i].real,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,Data[:,i].real,MLS,markersize=MMS,color=PYCOL[i])
    ...
    if Title==True:
        plt.title(r"Tensor coefficients of Re($\mathcal{C}$)")

    #Create the legend
    names = []
    for i in range(Nval):
        names.append(r"Re($\mathcal{C}_{"+str(I[i])+","+str(J[i])+","+str(K[i])+"}(\omega)$)")
    ...
    for i in range(Nval):
        if i==0:
            lines = ax.plot(Array,Data[:,i].imag,MLS,markersize=MMS,color=PYCOL[i])
        else:
            lines += ax.plot(Array,Data[:,i].imag,MLS,markersize=MMS,color=PYCOL[i])
    ymin, ymax = ax.get_ylim()
    ...
    if Title==True:
        plt.title(r"Tensor coefficients of $\mathcal{C}$")

    #Create the legend
    names = []
    for i in range(Nval):
        names.append(r"Im($\mathcal{C}_{"+str(I[i])+","+str(J[i])+","+str(K[i])+"}(\omega)$)")
...
def PODErrorPlotter(savename,Array,PODArray,Values,PODValues,Errors,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, _, TTP,MLS, MMS, SLS, SMS, EBLS, EBMS, ECL, IR3,JR3,KR3,IR4,JR4,KR4,LR4  = PlotterSettings()
...
def ErrorPlotter(savename,Array,Values,Errors,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, _, TTP, MLS, MMS, _, _, EBLS, EBMS, ECL, IR3,JR3,KR3,IR4,JR4,KR4,LR4 = PlotterSettings()
