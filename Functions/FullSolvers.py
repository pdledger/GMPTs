...
def FullSweepMultiR3(Object,Order,alpha,inorout,mur,sig,Array,CPUs,BigProblem):
    Object = Object[:-4]+".vol"
    #Set up the Solver Parameters
    Solver,epsi,Maxsteps,Tolerance = SolverParameters()
    ...
    #Set up how the tensor and eigenvalues will be stored
    N0=np.zeros([3,3])
    TensorArray=np.zeros([NumberofFrequencies,27], dtype=complex)
    RealEigenvalues = np.zeros([NumberofFrequencies,3])
    ImaginaryEigenvalues = np.zeros([NumberofFrequencies,3])
    EigenValues = np.zeros([NumberofFrequencies,3], dtype=complex)
...
#This section solves the Theta1 problem and saves the solution vectors
    ...
    #Run on the multiple cores
    with multiprocessing.Pool(Theta1_CPUs) as pool:
        Outputs = pool.starmap(Theta1_SweepR3, Runlist)
...
#of the snapshots
    ...
    #Solve the problem
    TensorArray, EigenValues = Theta1_SweepR3(Array,mesh,fes,fes2,Theta0Sol,xivec,alpha,sigma,mu,inout,Tolerance,Maxsteps,epsi,Solver,N0,NumberofFrequencies,False,True,False,False)
