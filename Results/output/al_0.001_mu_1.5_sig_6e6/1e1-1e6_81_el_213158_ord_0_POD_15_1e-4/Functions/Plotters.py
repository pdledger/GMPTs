...
def PODEigPlotter(savename,Array,PODArray,EigenValues,PODEigenValues,EddyCurrentTest):
    ...
    Title, Show, ETP, _, MLS, MMS, SLS, SMS, _, _, ECL, I,J,K = PlotterSettings()
...
def EigPlotter(savename,Array,EigenValues,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, ETP, _, MLS, MMS, _, _, _, _, ECL, I,J,K = PlotterSettings()
...
def PODTensorPlotter(savename,Array,PODArray,Values,PODValues,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, _, TTP,MLS, MMS, SLS, SMS, _, _, ECL, I,J,K = PlotterSettings()
    ...
    PODData=np.zeros([PODPoints,Nval],dtype=complex)
    for nval in range(Nval):
        ...
        for nf in range(PODPoints):
            PODData[nf,nval] = PODValues[nf,index-1]
    ...
    #Plot the snapshots
    for i in range(Nval):
        lines += ax.plot(PODArray,PODData[:,i].real,SLS,markersize=SMS,color=PYCOL[i])
    ...
    names = []
    for i in range(Nval):
        names.append(r"Re($\mathcal{C}_{"+str(I[i])+","+str(J[i])+","+str(K[i])+"}(\omega)$) POD")
    for i in range(Nval):
        names.append(r"Re($\mathcal{C}_{"+str(I[i])+","+str(J[i])+","+str(K[i])+"}(\omega)$) Snapshot")
    ...
    #Plot the snapshots
    for i in range(Nval):
        lines += ax.plot(PODArray,PODData[:,i].imag,SLS,markersize=SMS,color=PYCOL[i])
    ...
    if Title==True:
        plt.title(r"Tensor coefficients of Im($\mathcal{C}$)")

    #Create the legend
    names = []
    for i in range(Nval):
        names.append(r"Im($\mathcal{C}_{"+str(I[i])+","+str(J[i])+","+str(K[i])+"}(\omega)$) POD")
    for i in range(Nval):
        names.append(r"Im($\mathcal{C}_{"+str(I[i])+","+str(J[i])+","+str(K[i])+"}(\omega)$) Snapshot")
...
def TensorPlotter(savename,Array,Values,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, _, TTP, MLS, MMS, _, _, _, _, ECL, I,J,K = PlotterSettings()
...
def PODErrorPlotter(savename,Array,PODArray,Values,PODValues,Errors,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, _, TTP,MLS, MMS, SLS, SMS, EBLS, EBMS, ECL, I,J,K  = PlotterSettings()
...
def ErrorPlotter(savename,Array,Values,Errors,EddyCurrentTest):
    ...
    #Retrieve the settings for the plot
    Title, Show, _, TTP, MLS, MMS, _, _, EBLS, EBMS, ECL, I,J,K = PlotterSettings()
